...
import { Filter, Search, CloudSun, Calendar, Shirt, Trash2, Shirt as ShirtIcon, X, MousePointer2, Loader2 } from "lucide-react";
...
export default function LaundryClient({ initialClothes }: { initialClothes: ClothingItem[] }) {
  const [clothes, setClothes] = useState(initialClothes);
  const [prevInitialClothes, setPrevInitialClothes] = useState(initialClothes);

  // Synchronize state with props immediately during the render phase
  ...
  const dynamicTypeOptions = useMemo(() => {
    ...
    const standardOrder = ["top", "bottom", "one-piece", "shoes"];
  ...
  const filteredClothes = useMemo(() => {
    return clothes.filter(item => {
      ...
      const matchesFilter = activeFilter === "all" || itemGroup === activeFilter.toLowerCase();
      ...
      return matchesFilter && matchesWeather && matchesCategory && matchesSearch;
    });
  ...
  const handleBulkWardrobe = () => {
    const ids = Array.from(selectedIds);
    if (ids.length === 0) return;

    startTransition(async () => {
      try {
        await bulkUpdateStatus(ids, "available");
        setClothes(prev => prev.filter(c => !selectedIds.has(c.id)));
        setSelectedIds(new Set());
        setIsSelectionMode(false);
        toast(`Moved ${ids.length} items back to wardrobe`, "success");
        router.refresh();
      } catch (err) {
  ...
  const handleBulkDelete = () => {
    ...
    startTransition(async () => {
      try {
        ...
        toast(`Successfully removed ${ids.length} items`, "success");
        router.refresh();
      } catch (err) {
        toast(err instanceof Error ? err.message : "Failed to remove items", "error");
      }
    });
  };
  ...
  return (
    <div className="space-y-8 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between flex-1 gap-6">
          <div>
            <h1 className="text-4xl font-black tracking-tight text-text-primary">Laundry Basket</h1>
            <p className="text-text-muted mt-2 font-medium">Items currently being refreshed.</p>
          </div>
          ...
          <button
            ...
            className={`px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest border transition-all flex items-center gap-2 ${isSelectionMode ? 'bg-text-primary text-background border-white shadow-xl scale-105' : 'bg-text-primary/5 text-text-muted border-border-color hover:border-border-color/20'}`}
          >
            {isSelectionMode ? <X className="w-4 h-4" /> : <MousePointer2 className="w-4 h-4" />}
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full sm:w-72 bg-text-primary/5 border border-border-color text-text-primary text-sm rounded-2xl py-3.5 pl-12 pr-5 focus:outline-none focus:border-white/30 focus:ring-1 focus:ring-white/30 transition-all placeholder:text-text-primary/20"
          />
        </div>
      </div>
      ...
      {filteredClothes.length === 0 && (
        <div className="text-center py-24 sm:py-32 bg-white/[0.02] border border-border-color rounded-[3rem] shadow-2xl flex flex-col items-center justify-center gap-6 animate-in fade-in zoom-in duration-700">
          ...
          <div className="space-y-2">
            <p className="text-text-primary text-2xl font-black tracking-tight">No Clothes Found</p>
            <p className="text-text-muted text-sm font-medium max-w-[200px] mx-auto">Your laundry basket is looking empty or filters are too strict.</p>
          </div>
          <button
            onClick={() => { setActiveFilter('all'); setActiveWeather('all'); setSearchQuery(''); }}
      ...
      {isSelectionMode && selectedIds.size > 0 && (
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-[100] w-[calc(100%-2rem)] max-w-lg animate-in slide-in-from-bottom-10 duration-500">
          <div className="bg-background-secondary/90 backdrop-blur-2xl border border-border-color/20 rounded-[2.5rem] p-4 shadow-custom flex items-center justify-between gap-4">
            <div className="flex flex-col ml-4">
              <span className="text-text-primary font-black text-lg leading-none">{selectedIds.size}</span>
              <span className="text-text-muted text-[9px] font-black uppercase tracking-widest">Selected</span>
            </div>

            <div className="flex items-center gap-2">
              <button
                onClick={handleBulkWardrobe}
                disabled={isPending}
                className="bg-green-500 text-text-primary px-6 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center gap-2 hover:scale-105 active:scale-95 transition-all shadow-xl shadow-green-500/20 disabled:opacity-50"
              >
                {isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <ShirtIcon className="w-4 h-4" />}
                To Wardrobe
              </button>
              ...
              <button
                ...
                disabled={isPending}
                className="bg-red-500 text-text-primary px-6 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center gap-2 hover:scale-105 active:scale-95 transition-all shadow-xl shadow-red-500/20 disabled:opacity-50"
              >
      ...
      {showBulkDeleteConfirm && (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-background/90 backdrop-blur-xl p-4 animate-in fade-in zoom-in-95 duration-300" onClick={() => setShowBulkDeleteConfirm(false)}>
          <div className="bg-card-background border border-border-color rounded-[3rem] p-10 w-full max-w-sm shadow-2xl text-center" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-text-muted mb-10 text-lg font-medium leading-relaxed">This will permanently remove these items from your laundry basket. This action cannot be undone.</p>
